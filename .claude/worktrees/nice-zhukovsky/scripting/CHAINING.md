...
In Grasshopper, components connect via wires. A wire carries data from one component's output to another's input. For two ScriptNode components to chain together, the upstream script's output must produce a type that the downstream script's input can consume.

ScriptNode outputs are untyped (generic). ScriptNode inputs are typed (via the header). Grasshopper will attempt to auto-cast the generic output to the expected input type. This works reliably when the actual Python object matches the declared type. It fails when there's a genuine type mismatch.
...
Each output can feed a different downstream ScriptNode or native GH component.

### Pattern 4: Mixing ScriptNodes with native GH
ScriptNode outputs are generic but Grasshopper handles the casting. You can wire a ScriptNode output directly into a native GH component's input:

- ScriptNode outputting `list[Point3d]` → native `Voronoi` component's `Points` input: works
- ScriptNode outputting `list[Curve]` → native `Loft` component's `Curves` input: works
- Native `Divide Curve` component's `Points` output → ScriptNode input `points:list[Point3d]`: works

This is a major advantage of the ScriptNode approach — you can mix algorithmic Python scripts with the full Ladybug/Kangaroo/native GH ecosystem.
...
**When a ScriptNode outputs a flat list** (e.g., `points = [pt1, pt2, pt3]`), it creates a single-branch tree `{0}` with 3 items.

**When a ScriptNode outputs a nested list** (e.g., `points = [[pt1, pt2], [pt3, pt4]]`), it creates a two-branch tree `{0}` and `{1}`, with 2 items each.

**When a downstream ScriptNode has `list[Point3d]` input**, it receives one branch at a time. If the upstream tree has multiple branches, the downstream script runs once per branch.

**When a downstream ScriptNode has `Point3d` (item access) input**, it receives one item at a time. GH iterates through every item in every branch.
