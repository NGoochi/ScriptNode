...
It lives in the **Script > ScriptNode** tab alongside ScriptNode.
...
| Dynamic script I/O | ScriptNode |

**Primary use case:** Configuring per-item data for list-based ScriptNode inputs. For example, defining per-level heights, offsets, and floor areas for a 14-story building — all from one node with one set of sliders.
...
- Match the downstream ScriptNode input name exactly — the DataNode output name IS the field name
...
## Connecting DataNode to ScriptNode

### The Pattern
```
DataNode (output: level_heights)  →  ScriptNode (input: level_heights : float list[])
DataNode (output: z_offsets)      →  ScriptNode (input: level_z_offsets : float list[])
```
...
**Key:** The ScriptNode input must have `list[]` in the type hint to accept a list from DataNode.
...
- The ScriptNode receives these as Python lists: `level_heights = [5500.0, 4200.0, 4000.0, ...]`
...
| File | Purpose |
|---|---|
| `DataNodeModels.cs` | Data model: `FieldDef`, `ItemRecord`, `OverrideKey`, `DataNodeSchema`, `SliderPresets` |
| `DataNodeComponent.cs` | GH component: SolveInstance, parameter rebuild, serialisation, MCP registration |
| `DataNodeAttributes.cs` | On-canvas rendering: card-style with blue header, MCP dot, Edit Data button |
| `DataNodeEditor.cs` | Eto.Forms editor: GridView-based, non-modal, singleton pattern |
| `Tools/DataNodeTool.cs` | MCP tools: `get_datanode_info`, `set_datanode_values`, `add_datanode_items`, `set_datanode_schema` |
